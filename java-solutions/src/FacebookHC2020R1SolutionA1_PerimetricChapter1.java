...
//JMR Template
public class FacebookHC2020R1SolutionA1_PerimetricChapter1 {
	...
	public static void solve() throws IOException{
		...
		long maxH = H[0];
		long minH = H[0];
		boolean desc = H[0]>H[1];
		long perimAcum = 0;
		long perimAdic = 0;
		int lastInitialI = 0;
		for (int i = 1; i < N; i++) {
			long perimAdicTmp = perimAdic;
			long minMaxH = minH;
			...
			if(L[i-1]+W<L[i]) {
				...
				if(!desc) {
					...
					perimAdicTmp += 2*(minMaxH-minH);
					if(i<N-1 && H[i]>H[i+1]) {
						perimAdic = perimAdicTmp;
						minH = minMaxH;
					}
				}
			}
			...
			P[i] = perimAcum + 2*(widthCurrentP+maxH) + perimAdicTmp;
